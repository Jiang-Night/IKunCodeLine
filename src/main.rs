use ikuncodeline::cli::Cli;
use ikuncodeline::config::{Config, InputData};
use ikuncodeline::core::{collect_all_segments, StatusLineGenerator};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.init {
        use ikuncodeline::config::InitResult;
        match Config::init()? {
    ...
    if cli.print {
        ...
        if let Some(theme) = cli.theme {
            config = ikuncodeline::ui::themes::ThemePresets::get_theme(&theme);
        }
    ...
    if cli.config {
        #[cfg(feature = "tui")]
        {
            ikuncodeline::ui::run_configurator()?;
        }
        #[cfg(not(feature = "tui"))]
        {
    ...
    if let Some(claude_path) = cli.patch {
        use ikuncodeline::utils::ClaudeCodePatcher;
    ...
    if io::stdin().is_terminal() {
        ...
        #[cfg(feature = "tui")]
        {
            use ikuncodeline::ui::{MainMenu, MenuResult};
        ...
        #[cfg(not(feature = "tui"))]
        {
            ...
            eprintln!("Usage: echo '{{...}}' | ikuncodeline");
            eprintln!("   or: ikuncodeline --help");
        }
        return Ok(());
    }
